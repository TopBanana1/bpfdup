...
# --- helpers ---
normalize_hex(){ local s="$1"; s="${s//:/}"; s="${s// /}"; s="${s//0x/}"; s="${s//0X/}"; s=$(echo -n "$s"|tr 'A-F' 'a-f'|tr -cd '0-9a-f'); echo -n "$s"; }
to_hex(){ printf '%s' "$1"|xxd -p -c 256|tr -d '\n'|tr 'A-F' 'a-f'; }
is_hex_even(){ [[ "$1" =~ ^[0-9a-f]+$ ]] && (( ${#1} % 2 == 0 )); }
strip_nuls(){ tr -d '\000' <<<"$1"; }
ascii_one_line(){ strip_nuls "$1"|tr '\r' ' '|tr '\t' ' '|tr -cd '\40-\176'|awk '{printf "%s",$0}'; }
...
build_essid_map() {
  mapfile -t raw_pairs < <(
    ...
    | awk -F'\t' '{
        ...
        if (b!="" && b!="ff:ff:ff:ff:ff:ff" && b!="00:00:00:00:00:00") print b "\t" s
      }' \
    | sort -u
  )
  ...
  if [[ ${#essid_to_bssids[@]} -eq 0 && ${#label_to_bssids[@]} -eq 0 ]]; then
    mapfile -t raw2 < <(
      tshark -r "$file" -Y "$MGMT_Y" \
    ...
    for line in "${raw2[@]:-}"; do
      bssid=${line%%$'\t'*}; s=${line#*$'\t'}; [[ -z "$s" ]] && continue
      nh=$(normalize_hex "$s")
      if [[ -n "$nh" ]]; then
        if is_hex_even "$nh"; then
          essid_to_bssids["$nh"]+="$bssid"$'\n'
        ...
        essid_to_bssids["$s"]+="$bssid"$'\n'
        label="$(make_label_ascii_pref "$s")"
      fi
      label_to_bssids["$label"]+="$bssid"$'\n'
    done
  fi
...
if [[ ${#essids[@]} -gt 0 ]]; then
  for want in "${essids[@]}"; do
    matched=0
    if [[ -n "${essid_to_bssids[$want]+x}" ]]; then while IFS= read -r b; do [[ -n "$b" ]] && bssid_set["$b"]=1; done <<<"${essid_to_bssids[$want]}"; matched=1; fi
    if (( !matched )); then hw=$(to_hex "$want"); if [[ -n "${essid_to_bssids[$hw]+x}" ]]; then while IFS= read -r b; do [[ -n "$b" ]] && bssid_set["$b"]=1; done <<<"${essid_to_bssids[$hw]}"; matched=1; fi; fi
    if (( !matched )); then nw=$(normalize_hex "$want"); if [[ -n "$nw" && -n "${essid_to_bssids[$nw]+x}" ]]; then while IFS= read -r b; do [[ -n "$b" ]] && bssid_set["$b"]=1; done <<<"${essid_to_bssids[$nw]}"; matched=1; fi; fi
    if (( !matched )); then mapfile -t fb < <(resolve_bssids_from_raw_ssid "$file" "$want" || true); if [[ ${#fb[@]} -gt 0 ]]; then for b in "${fb[@]}"; do [[ -n "$b" ]] && bssid_set["$b"]=1; done; matched=1; fi; fi
    (( matched )) || echo "Warning: ESSID not found (ascii/hex): $want" >&2
  done
else
  if [[ ${#label_to_bssids[@]} -eq 0 ]]; then
    echo "No SSIDs present in capture — falling back to BSSID-only selection." >&2
    mapfile -t only_bssids < <(
      tshark -r "$file" -Y '(wlan.fc.type == 0)' -T fields -e wlan.bssid \
      | awk '{
          b=tolower($0);
    ...
    maxlen=0; for L in "${labels[@]}"; do len=${#L}; (( len > maxlen )) && maxlen=$len; done
    mapfile -t display_rows < <(
      for L in "${labels[@]}"; do
        cnt=$(printf '%s' "${label_to_bssids[$L]}" | sed '/^$/d' | wc -l | awk '{print $1}')
    ...
    for L in "${picked_labels[@]}"; do while IFS= read -r b; do [[ -n "$b" ]] && bssid_set["$b"]=1; done <<<"${label_to_bssids[$L]}"; done
...
band_letter_for_freq(){ local f="$1"; if   [[ "$f" -ge 2400 && "$f" -le 2500 ]]; then echo -n "a"
elif [[ "$f" -ge 5000 && "$f" -le 5900 ]]; then echo -n "b"
elif [[ "$f" -ge 5925 && "$f" -le 7125 ]]; then echo -n "c"
elif [[ "$f" -ge 57000 && "$f" -le 71000 ]]; then echo -n "d"
elif [[ "$f" -ge 800 && "$f" -le 1000 ]]; then echo -n "e"; else echo -n ""; fi; }
channel_from_freq(){ local f="$1"; if   [[ "$f" -ge 2412 && "$f" -le 2472 ]]; then echo $(( (f - 2412)/5 + 1 ))
elif [[ "$f" -eq 2484 ]]; then echo "14"
elif [[ "$f" -ge 5000 && "$f" -le 5900 ]]; then echo $(( (f - 5000)/5 ))
elif [[ "$f" -ge 5925 && "$f" -le 7125 ]]; then echo $(( (f - 5955)/5 + 1 )); else echo ""; fi; }
...
collect_from_lines(){
  while IFS=$'\t' read -r f1 f2 f3; do
    ...
    if [[ -n "$freq" ]]; then
      ...
      if [[ -n "$ch" ]]; then
        chan_set["$ch"]=1
        bletter="$(band_letter_for_freq "$freq")"; [[ -n "$bletter" ]] && chan_banded_set["${ch}${bletter}"]=1
      fi
    else
      ch="${f3:-}"; [[ -n "$ch" && "$ch" =~ ^[0-9]+$ ]] && chan_set["$ch"]=1
    fi
  done
}
...
for mac in "${SELECTED_BSSIDS[@]}"; do
  ...
  # Fallback: any management frames for this BSSID
...
for c in "${!chan_set[@]}"; do
  if (( include_dfs )); then
    ...
    if is_dfs "$c"; then removed_dfs=1; continue; else chan_filtered_set["$c"]=1; fi
  fi
done

channel_list=""; banded_list=""; freq_list=""
((${#chan_filtered_set[@]} > 0)) && channel_list=$(printf '%s\n' "${!chan_filtered_set[@]}" | sort -n | paste -sd, -)
((${#chan_banded_set[@]} > 0))   && banded_list=$(printf '%s\n' "${!chan_banded_set[@]}" | sort -V | paste -sd, -)
((${#freq_set[@]} > 0))          && freq_list=$(printf '%s\n' "${!freq_set[@]}" | sort -n | paste -sd, -)

# --- BPF compile ---
build_clause(){ local mac="$1" mode="$2"; local m=$(echo "$mac"|tr 'A-F' 'a-f'); if [[ "$mode" == "3" ]]; then printf 'wlan addr3 %s' "$m"; else printf '(wlan addr1 %s or wlan addr2 %s or wlan addr3 %s or wlan addr4 %s)' "$m" "$m" "$m" "$m"; fi; }
build_expr_for_bssids(){ local mode="$1"; shift; local out="" clause; for mac in "$@"; do clause=$(build_clause "$mac" "$mode"); out=${out:+"$out or "}$clause; done; printf '%s' "$out"; }
compile_expr_to_bcf(){ local expr="$1" out="$2"; local filterfile="$TMPDIR/filter.expr"; printf '%s\n' "$expr" > "$filterfile"; tcpdump -s 1024 -y IEEE802_11_RADIO -F "$filterfile" -ddd > "$out" 2>"$TMPDIR/tcpdump.err"; }
...
# --- BPF compile flow (with chunking fallback) ---
mapfile -t ALL_BSSIDS < <(printf '%s\n' "${!bssid_set[@]}" | sort -V)

try_mode="$addr_mode"; need_chunk=0; compiled_parts=()
expr=$(build_expr_for_bssids "$try_mode" "${ALL_BSSIDS[@]}")
if compile_expr_to_bcf "$expr" "$bcf_base"; then compiled_parts=( "$bcf_base" )
else
  if [[ "$try_mode" == "all" ]]; then
    try_mode="3"; expr=$(build_expr_for_bssids "$try_mode" "${ALL_BSSIDS[@]}")
    if compile_expr_to_bcf "$expr" "$bcf_base"; then compiled_parts=( "$bcf_base" ); else need_chunk=1; fi
  else need_chunk=1; fi
fi
if [[ "$need_chunk" == "1" ]]; then
  ...
  while (( i < ${#ALL_BSSIDS[@]} )); do
    ...
    while (( sz >= 1 )); do
      subset=( "${ALL_BSSIDS[@]:i:sz}" ); expr=$(build_expr_for_bssids "$try_mode" "${subset[@]}"); out="${file}.part$(printf '%02d' "$part").bcf"
      if compile_expr_to_bcf "$expr" "$out"; then compiled_parts+=( "$out" ); (( i += sz )); (( part++ )); success=1; (( local_batch = sz < 128 ? sz*2 : 128  )); break; else (( sz /= 2 )); fi
    done
    (( success )) || { echo "Failed to compile even a single-BSSID filter. Aborting." >&2; exit 1; }
  done
fi

# --- combined JSON (via python3) ---
# Use a dedicated subdir for list files and pass it to Python explicitly
...
# user-facing example (inline values, no .bands/.freqs files)
